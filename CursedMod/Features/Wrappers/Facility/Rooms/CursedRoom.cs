...
public class CursedRoom
{
    ...
    public static CursedRoom Get(Vector3 position) => RoomIdUtils.RoomAtPositionRaycasts(position, false) is { } roomIdentifier ? Get(roomIdentifier) : null;
    ...
    public Vector3 GetLocalPoint(Vector3 localPointOfRoom) => Transform.TransformPoint(localPointOfRoom);

    public Vector3 GetGlobalPoint(Vector3 globalPoint) => Transform.InverseTransformPoint(globalPoint);
    
    public bool IsInRoom(Vector3 position) => RoomIdUtils.RoomAtPositionRaycasts(position, false) == Room;
    
    public bool IsInRoom(GameObject gameObject) => IsInRoom(gameObject.transform.position);
