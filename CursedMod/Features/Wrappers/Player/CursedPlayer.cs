...
#pragma warning restore SA1401
    ...
    public string SaltedUserId => AuthManager.SaltedUserId;
    ...
    public bool BadgeHidden => !string.IsNullOrEmpty(ServerRoles.HiddenBadge);
    ...
    public string UserId
    {
        ...
        set => AuthManager.UserId = value;
    }
    
    public string SyncedUserId
    {
        get => AuthManager.SyncedUserId;
        set => AuthManager.NetworkSyncedUserId = value;
    }
    ...
    {
        get => AuthManager.InstanceMode;
        set => AuthManager.InstanceMode = value;
    }

    public string AuthToken => AuthManager.GetAuthToken();
    ...
    {
        get => AuthManager.DoNotTrack;
        set => AuthManager.DoNotTrack = value;
    }
    ...
    public bool IsNorthWoodStaff => AuthManager.NorthwoodStaff;

    public bool IsGlobalModerator => AuthManager.RemoteAdminGlobalAccess;
    ...
    public CursedRoom CurrentRoom => CursedRoom.Get(Position);
    ...
    public void OpenRemoteAdmin() => ServerRoles.TargetSetRemoteAdmin(true);

    public void CloseRemoteAdmin() => ServerRoles.TargetSetRemoteAdmin(false);
    ...
    public void SendCommandBind(KeyCode code, string command) => CharacterClassManager.TargetChangeCmdBinding(code, command);

    public void SendConsoleMessage(string text, string color) => GameConsoleTransmission.SendToClient(text, color);

    public void Disconnect(string message) => CharacterClassManager.DisconnectClient(NetworkConnection, message);
    
    public void ChangeTagVisibility() => ServerRoles.TryHideTag();
    ...
    public void AttachAdminToy(CursedAdminToy adminToy, Vector3 offset)
    {
        adminToy.Rotation = Rotation;
        Transform transform1;
        adminToy.Position = (transform1 = Transform).TransformPoint(offset);
        adminToy.GameObject.transform.parent = transform1;
    }
    ...
    public void Mute(bool isTemporary, bool isIntercom = false)
    {
        if (isTemporary)
        {
    ...
    public void Unmute(bool removeMute, bool isIntercom = false)
    {
        if (removeMute)
        {
